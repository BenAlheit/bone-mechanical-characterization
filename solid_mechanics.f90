...
module la
    doubleprecision, parameter :: ioi(6, 6) = reshape([ 1d0, 0d0, 0d0, 0d0, 0d0, 0d0, &
    ...
    doubleprecision, parameter :: ixi(6, 6) = reshape([ 1d0, 1d0, 1d0, 0d0, 0d0, 0d0, &
...
contains
    ...
    subroutine carraeu_visc_dsig_bar_e_deps_n1(mu, lambda, a, n, mu_0, mu_inf, eps_n1, eps_d_n, eps_d_n1, dsige_depsn1, dt)

        doubleprecision, intent(in) :: eps_n1(6), eps_d_n1(6), eps_d_n(6), mu, dt
        doubleprecision, intent(in) :: lambda, a, n, mu_0, mu_inf
        doubleprecision, intent(out) :: dsige_depsn1(6, 6)

        call dsig_bar_e_deps_n1(iso_lin_elastic_sig_d_depss, [mu], 1, &
                carraeu_viscd_sig_d_depsd_dot, [lambda, a, n, mu_0, mu_inf], 5, &
                eps_n1, eps_d_n, eps_d_n1, dsige_depsn1, dt)

    endsubroutine

    subroutine lin_visc_dsig_bar_e_deps_n1(mu, eta, eps_n1, eps_d_n, eps_d_n1, dsige_depsn1, dt)

        doubleprecision, intent(in) :: eps_n1(6), eps_d_n1(6), eps_d_n(6), mu, eta, dt
        doubleprecision, intent(out) :: dsige_depsn1(6, 6)

        call dsig_bar_e_deps_n1(iso_lin_elastic_sig_d_depss, [mu], 1, &
                lin_viscd_sig_d_depsd_dot, [eta], 1, &
                eps_n1, eps_d_n, eps_d_n1, dsige_depsn1, dt)

    endsubroutine

    subroutine dsig_bar_e_deps_n1(dsig_s_depse, sig_s_params, n_s, &
            dsig_d_depsd_dot, sig_d_params, n_d, &
            eps_n1, eps_d_n, eps_d_n1, dsige_depsn1, dt)
        use la

        integer, intent(in) :: n_s, n_d
        doubleprecision, intent(in) :: eps_n1(6), eps_d_n1(6), eps_d_n(6), sig_s_params(n_s), sig_d_params(n_d), dt
        doubleprecision :: eps_d_dot(6), eps_s(6)
        doubleprecision :: d_sig_d_depsd_dot_val(6, 6), d_sig_s_depss_val(6, 6)
        doubleprecision, intent(out) :: dsige_depsn1(6, 6)

        external :: dsig_s_depse, dsig_d_depsd_dot

        eps_d_dot = (eps_d_n1 - eps_d_n) / dt
        eps_s = eps_n1 - eps_d_n1

        call dsig_d_depsd_dot(d_sig_d_depsd_dot_val, sig_d_params, eps_d_dot, n_d)
        d_sig_d_depsd_dot_val = d_sig_d_depsd_dot_val / dt
        call dsig_s_depse(d_sig_s_depss_val, sig_s_params, eps_s, n_s)

        dsige_depsn1 = d_sig_d_depsd_dot_val
        dsige_depsn1 = matmul(dsige_depsn1, inv(d_sig_s_depss_val + d_sig_d_depsd_dot_val))
        dsige_depsn1 = matmul(dsige_depsn1, d_sig_s_depss_val)
        dsige_depsn1 = matmul(dsige_depsn1, ioi - ixi / 3d0)
...
program main
    ...
    doubleprecision :: eps_d_n1(6), sig_dn1(6), a, test(6, 6)
    !
    !
    !
