...
\begin{algorithm}[!htb]
	...
	\label{alg:comp-umat}
	\begin{algorithmic}[1]
		\Procedure{increment\_viscoelastic\_material}{$\bm{\varepsilon}$}
		\State $\bm{A}$(:, :, :) = \Call{CALC\_A}{$\bm{F}$, $\bm{a}_{0}$, $\bm{b}_{0}$}
		\State $\mathbb{A}$(:, :, :) = \Call{CALC\_BIG\_A}{$\bm{F}$, $\bm{a}_{0}$, $\bm{b}_{0}$}
		\State $J = \det \bm{F}$
		\State $\bm{\sigma} = \bm{0}$
